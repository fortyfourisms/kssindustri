...
export default function IKAS() {
    ...
    // ── Fetch SEMUA data IKAS → GET /api/maturity/ikas ─────────────────────
    const { data: ikasList, isLoading: ikasListLoading } = useQuery({
        queryKey: ["ikasList"],
    ...
    useEffect(() => {
        if (!ikasList || !Array.isArray(ikasList) || ikasList.length === 0) return;
