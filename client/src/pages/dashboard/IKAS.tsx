...
export default function IKAS() {
    ...
    // Default default structure mirroring Vue `ikasDataDynamic` fallback
    const [ikasDataDynamic, setIkasDataDynamic] = useState({
        total_rata_rata: 0,
    ...
    const uploadExcel = async (file: File) => {
        ...
        try {
            ...
            if (result.success) {
                // Here we might update table data based on result.data
                toast({ title: "Berhasil", description: "Upload berhasil!" });
            } else {
                            ...
                            <table className="w-full text-[12px] border-collapse bg-white">
                                <thead className="bg-[#f3f4f6] text-slate-600 text-[10px] uppercase font-extrabold tracking-wider border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th colSpan={5} className="border-b border-slate-200 p-2 text-center text-sm">2025</th>
                                    </tr>
