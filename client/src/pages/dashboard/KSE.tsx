...
import { kseCategories, getKategoriSE, type KseQuestion } from "@/data/kse-data";
import {
...
export default function KSE() {
    ...
    // Fetch existing SE data from API
    const { data: existingSe } = useQuery<any>({
        queryKey: ["se"],
        queryFn: api.getKse,
        enabled: !!user,
    });
    ...
    useEffect(() => {
        ...
        // Pre-fill respondent from user profile
        const p = user.perusahaan;

        // If API returned existing SE data, use it to pre-fill
        const se = Array.isArray(existingSe) ? existingSe[0] : existingSe;
        ...
        try {
            ...
            if (stored) {
                ...
                // Only merge fields that are empty from API
                ...
                // If respondent already exists, go to step 2
                if (saved.nama_se) {
                    setCurrentStep(2);
                }
            }
        ...
        // Load answers from existing SE data or localStorage
        if (se && !Array.isArray(existingSe)) {
            // Reconstruct answers from SE API response
            ...
            Object.entries(QUESTION_TO_FIELD).forEach(([qNo, field]) => {
                ...
                if (val && ['A', 'B', 'C'].includes(val)) {
                    // Find matching question to get bobot
            ...
            if (hasApiAnswers) {
                setAnswers(prev => {
                    // Only update if answers actually changed to prevent infant loops
                                    ...
                                    <div className="col-span-1 md:col-span-2">
                                        ...
                                        <form onSubmit={handleRespondentSubmit} noValidate>
                                            ...
                                            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
                                                <button
                                                    type="submit"
                                                    className="w-full md:w-auto px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold text-sm shadow-md shadow-blue-500/25
